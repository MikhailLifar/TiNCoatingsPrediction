...
def addDescriptors(sample, descriptors):
    ...
    for d in descriptors:
        ...
        if typ == 'stableExtrema':
            ...
            if 'Group' in typ:
                count = np.zeros(sample.getLength())
                centroid = np.zeros(sample.getLength())
                centroid[:] = np.nan
                mean_e = np.zeros(sample.getLength())
                mean_e[:] = np.nan
                mean_i = np.zeros(sample.getLength())
                mean_i[:] = np.nan
            else:
            ...
            energyInterval = params['energyInterval'] if 'energyInterval' in params else [sample.energy[0], sample.energy[-1]]
            for i in range(sample.getLength()):
                ...
                sp.limit(interval=energyInterval, inplace=True)
            ...
            if 'plotFolder' in params:
                ...
                for i in plot_inds:
                    plotting.plotToFile(sample.energy, sample.spectra.loc[i].to_numpy(), 'spectrum', sample.energy, arctan_y[i], 'arctan', fileName=f'{plotFolder}/{i}.png', title=f'efermi={efermi[i, 0]:.1f} efermiRate={efermi[i, 1]}')
            sample.addParam(paramName=f'{name}_e', paramData=efermi[:, 0])
            sample.addParam(paramName=f'{name}_slope', paramData=efermi[:, 1])
        elif typ == 'pca':
            assert set(params.keys()) <= {'usePcaPrebuildData', 'count', 'fileName', 'energyInterval'}, 'Wrong param names: '+str(set(params.keys()) - {'usePcaPrebuildData', 'count', 'fileName', 'energyInterval'})
...
def getQuality(sample, features, label_names, makeMixtureParams=None, model_class=None, model_regr=None, m=1, cv_count=10, returnModels=False, printDebug=True):
    ...
    if printDebug: print('')
    result = {label:{'quality':quality[label], 'predictedLabels':predictedLabels[label], 'trueLabels':trueLabels[label], 'quality_std':quality_std[label]} for label in quality}
    if returnModels:
...
def plotDescriptors2d(data, descriptorNames, labelNames, labelMaps=None, folder_prefix='', unknown=None, markersize=None, textsize=None, alpha=None, cv_count=2, plot_only='', doNotPlotRemoteCount=0, textColumn=None, additionalMapPlotFunc=None, cmap='seaborn husl', edgecolor=None, textcolor=None, linewidth=None, dpi=None, plotPadding=0.1, minQualityForPlot=None, model_class=None, model_regr=None):
    ...
    if plot_only != 'data':
        qualityRes = getQuality(data, descriptorNames, labelNames, m=1, cv_count=cv_count, model_class=model_class, model_regr=model_regr, returnModels=True)
    ...
    for label in labelNames:
        ...
        if plot_only == 'data':
            ...
            fileName2 = folder2 + os.sep + label + os.sep + f'{quality:.2f} {descriptorNames[0]}  {descriptorNames[1]}'
        ...
        if isClassification:
            if label in labelMaps: ticks = np.array(list(labelMaps[label].values()))
        ...
        if unknown is not None:
            ...
            ax.scatter(unknown[descriptorNames[0]], unknown[descriptorNames[1]], s=umarkersize ** 2, **c_params, vmin=0, vmax=1, edgecolor='black')
            for i in range(len(unknown)):
                ...
                ax.text(unknown.loc[i, descriptorNames[0]], unknown.loc[i, descriptorNames[1]], name, ha='center', va='center', size=umarkerTextSize, color=textcolor)
        ...
        if textsize>0:
            for i in range(data.shape[0]):
                ...
                ax.text(data.loc[i, descriptorNames[0]], data.loc[i, descriptorNames[1]], str(name), ha='center', va='center', size=textsize, color=textcolor)
    ...
    for label in qualityResult:
        ...
        def plot(true, pred, quality, filePostfix='', unkn_true=None, unkn_pred=None):
            ...
            if ML.isClassification(true):
                ...
                plotting.scatter(pred, true, color=err, alpha=alpha, markersize=markersize, text_size=textsize, marker_text=text, xlabel='predicted ' + label, ylabel='true ' + label, title='CV result for label ' + label + f'. Quality = {quality:.2f}', fileName=plotFileName + '.png', plotMoreFunction=plotMoreFunction)
                if plot_diff:
...
def calcPreedge(sample:ML.Sample, usePositiveConstrains=True, specialParams=None, plotFolder=None, debug=False, inplace=False):
    """
    :param specialParams: dict{ name: dict of substractBase params for spectrum}, if name == 'all' - apply to all spectra
    """
    def getParams(name):
        if specialParams is None: return None
        if 'all' in specialParams:
            assert len(specialParams) == 1
            return copy.deepcopy(specialParams['all'])
        if name in specialParams: return copy.deepcopy(specialParams[name])
        else: return None
    ...
    preedgeSpectra = []
    for i in range(sample.getLength()):
        sp = sample.getSpectrum(i)
        name = sample.params.loc[i,sample.nameColumn] if sample.nameColumn is not None else str(i)
        # if name != 'LC52_TiC5H5C7H7': continue
        if debug: print(name)
        plotFileName = f'{plotFolder}/{name}.png' if plotFolder is not None else None
        params = getParams(name)
        if params is None:
            res = curveFitting.substractBaseAuto(sp.x, sp.y, usePositiveConstrains=usePositiveConstrains, plotFileName=plotFileName, sepFolders=True, debug=debug)
        else:
            if 'plotFileName' not in params: params['plotFileName'] = plotFileName
            res = curveFitting.substractBase(sp.x, sp.y, **params)
        peak = res['peak']
    ...
    sample.addSpectrumType(preedgeSpectra, spType='pre-edge')
