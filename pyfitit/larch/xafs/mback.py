...
def match_f2(p, en=0, mu=1, f2=1, e0=0, em=0, weight=1, theta=1, order=None,
    ...
    if leexiang:
        func = func / (p['s']*mu)  # in original code there are no brackets
    return func
...
def mback(energy, mu=None, group=None, z=None, edge='K', e0=None, pre1=None, pre2=-50,
    ...
    if np.median(mu[n1:(n2+1)])/np.median(mu[p1:(p2+1)]) > 20:
        leexiang = False
    ...
    params = Parameters()
    ratio = np.median((f2-np.min(f2))/(mu-np.min(mu)))
    # print('ratio =', ratio)
    params.add(name='s',  value=1.0,  vary=True, min=0, max=ratio*5)  # scale of data
    params.add(name='xi', value=50.0, vary=False, min=0) # width of erfc
    # params.add(name='xi', value=50.0, vary=False, min=0, max=200) # width of erfc
    params.add(name='a',  value=0.0, vary=False)  # amplitude of erfc
    # params.add(name='a',  value=0.0, vary=False, min=0, max=10)  # amplitude of erfc
    if fit_erfc:
        params['a'].vary  = True
        params['a'].value = 0.5
        params['xi'].vary  = True
    ...
    for i in range(order+1): # polynomial coefficients
        ...
        # params.add(name='c%d' % i, value=0, vary=True, min=-10**(-i), max=10**(-i))
    ...
    # kws = dict(en=energy, mu=mu, f2=f2, e0=e0, em=em, order=order, weight=weight, theta=theta, leexiang=leexiang)
    # import scipy, scipy.optimize
    # names = [params[p].name for p in params]
    # start = [params[p].value for p in names]
    # bounds = [[params[p].min,params[p].max] for p in names]
    # print(names,'\n',bounds)
    # def func(v):
    #     p = dict(zip(names,v))
    #     return np.linalg.norm(match_f2(p, **kws))
    # res = scipy.optimize.minimize(func, start, bounds=bounds)
    # print(res)
    # opars = dict(zip(names, res.x))
    # print(opars)

    opars = out.params.valuesdict()
    eoff = energy - e0

    norm_function = opars['a']*erfc((energy-em)/opars['xi']) + opars['c0']  # erfc function + constant term
    for i in range(order):  # successive orders of polynomial
        attr = 'c%d' % (i + 1)
        if attr in opars:
    ...
    # print('fpp =', group.fpp)
    # print('theta =', theta)
    # print('weight =', weight[theta==1])
    # print('resudual =', np.abs(f2-group.fpp) * theta / weight)
    # print(out.message, out.residual[theta==1])
    ...
    group.loss = np.linalg.norm(out.residual[theta==1], ord=1)
