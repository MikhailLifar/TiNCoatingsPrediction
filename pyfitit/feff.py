...
def getPotentials(molecule, absorber, separatePotentials):
    """Generate ipot column for the atoms in the molecule and ipot list"""
    if separatePotentials:
        molecule = copy.deepcopy(molecule)
        rad = 3
        bool_ind = np.linalg.norm(molecule.atom-molecule.atom[[absorber]], axis=1) < rad
        ind = np.where(bool_ind)[0]
        other = np.where(~bool_ind)[0]
        otherNames = molecule.atomName[other]
        # print(molecule.atomName[ind], otherNames)
        for i in ind:
            if i == absorber: continue
            if molecule.atomName[i] not in otherNames: continue
            if molecule.az[i] == 1: continue
            molecule.atomName[i] = molecule.atomName[i]+'_'+str(i)
            # print(molecule.atomName[i])
        return getPotentials(molecule, absorber, separatePotentials=False)
...
def generateInputUniqueAbsorber(molecule, folder, additional, absorber, feffVersion, separatePotentials, **cards):
    ...
    with open(folder + os.sep + 'feff.inp', 'w') as f:
        ...
        ipot, ipotList = getPotentials(molecule, absorber, separatePotentials)
        for ip in ipotList:
