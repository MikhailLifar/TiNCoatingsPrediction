...
def parseAllFolders(parentFolder, parserKind, printOutput=True):
    """
    ...
    :param parserKind: one of 'fdmnes', 'fdmnesTest', 'adf', 'feff 6/8.5/9', 'pyGDM'. User can define custom folder parser. In this case he should set parserKind to dict with keys:
            'parseOneFolder' - function(folder)
            'makeCommonEnergy' - str for getCommonEnergyBuilder or function(allEnergies). In this case ordinary createDataframes function is used
    ...
    def separate_folders(allData):
        ...
        """
        badFolders = []
        if parserKind in ['fdmnes', 'fdmnesTest']:
        ...
        for d in allData:
            ...
            if parserKind in ['fdmnes', 'fdmnesTest'] and len(allData[d].x) != maxEnergyCount:
    ...
    if isinstance(parserKind, str):
        parseOneFolder = getParser(parserKind)
    else:
        parseOneFolder = parserKind['parseOneFolder']
    output = []
    ...
    if not isinstance(parserKind, str):
        # custom
        if 'createDataframes' in parserKind:
            df_spectra = parserKind['createDataframes'](allData, parentFolder, goodFolders)
            if isinstance(df_spectra, tuple):
                df_spectra, goodFolders, badFolders = df_spectra
        else:
            if 'makeCommonEnergy' in parserKind:
                makeCommonEnergy = parserKind['makeCommonEnergy']
                if isinstance(makeCommonEnergy, str): makeCommonEnergy = getCommonEnergyBuilder(makeCommonEnergy)
            else: makeCommonEnergy = getCommonEnergyBuilder('default')
            df_spectra, goodFolders, badFolders = createDataframes(allData, parentFolder, goodFolders, makeCommonEnergy)
        paramNames, _ = getParams(os.path.join(parentFolder, goodFolders[0], 'params.txt'))
    ...
    elif parserKind == 'pyGDM':
        df_abs, df_params = createDataframes({f:d['abs'] for f,d in allData.items() if f in goodFolders}, parentFolder, goodFolders, getCommonEnergyBuilder(parserKind))
        df_ext, _ = createDataframes({f:d['ext'] for f,d in allData.items() if f in goodFolders}, parentFolder, goodFolders, getCommonEnergyBuilder(parserKind))
        df_spectra = {'abs': df_abs, 'ext': df_ext}
    elif parserKind == 'vasp_rdf_energy':
        allData1 = {}
        for f in allData:
        ...
        df_spectra, df_params = createDataframes(allData1, parentFolder, goodFolders, getCommonEnergyBuilder(parserKind))
        vasp_energies = []
...
def convertToSample(dataset, spectralProgram):
    ...
    df_xanes, df_params, good, _ = parseAllFolders(parentFolder=os.path.split(additionalData[good[0]]['folder'])[0], parserKind=spectralProgram, printOutput=False)
    if len(good) == 0: return None, None
