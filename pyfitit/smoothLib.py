...
def simpleSmooth(e, xanes, sigma, kernel='Cauchy', new_e=None, sigma2percent=0.1, gaussWeight=0.2, assumeZeroInGaps=False, expandParams=None, debugPlotFile=None):
    ...
    if debugPlotFile is not None:
        plotting.plotToFile(e,xanes,'expand', e0,xanes0,'init', new_e, new_xanes, 'smoothed', fileName=debugPlotFile)
    return new_xanes
...
def smoothDataFrame(smoothParams, xanes_df, smoothType, expSpectrum, fitNormInterval, norm=None, folder=None, returnCacheStatus=False, energy=None):
    ...
    if folder is not None:
        ...
        if os.path.isfile(smoothFileName) and os.path.isfile(smoothParamsFileName):
            ...
            if len(smoothParams) == len(smoothParamsCached):
                ...
                if not np.array_equal(en, expSpectrum.energy): equal = False
                if not np.array_equal(exp_xanes, expSpectrum.intensity): equal = False
            else: equal = False
            if equal:
    ...
    if xanes_df_is_dataframe:
        ...
        if folder is not None:
            ...
            with open(smoothParamsFileName, 'w') as f: json.dump({'smoothParams':smoothParams, 'energy':expSpectrum.energy.tolist(), 'exp':expSpectrum.intensity.tolist()}, f)
...
def smoothExp(s, checkInterval=None, bw=None, maxw=4, dw=1, expandParams=None, debug=False):
    """
    Iteratively applies removeNoise function to the sspectrum with increased bandwidth from 1 up to
