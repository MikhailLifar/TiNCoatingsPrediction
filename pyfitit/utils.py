import scipy, math, random, string, os, importlib, pathlib, matplotlib, ipykernel, urllib, json, copy, glob, numbers, itertools, pickle, time, re, warnings, sys, types, subprocess, shlex, jupytext, nbformat, scipy.stats, scipy.spatial, io, platform, shutil
...
class Spectrum(object):
    ...
    def toExafs(self, Efermi, k_power):
        ...
        regular_k = np.linspace(k[0], k[-1], int((k[-1]-k[0])*100))
        intSqr = np.interp(regular_k, k, intSqr)
        return Spectrum(regular_k, intSqr, xName='k', yName='chi')
    ...
    def makeCommonX(self, other):
        ...
        if np.sum(ind1) > np.sum(ind2):
            ...
            e = other.x[ind2]
            s1,s2 = np.interp(e, self.x, self.y), other.intensity[ind2]
        return e, s1, s2
...
def read_data(file, skiprows=0, separator=r'\s+', decimal=".", guess=True):
    ...
    if guess:
        ...
        if header is not None:
            ...
            if len(header) != ncols:
                if len(header) == ncols+1 and header[0] in ['#', '!', ';', '//']:
                    header = header[1:]
                else:
                    header = None
        for i in range(len(lines)):
            numbers = re.findall(r'[+-]?(?:\d+(?:\.\d*)?|\.\d+)(?:[eE][+-]?\d+)?', lines[i])
            if len(numbers) != ncols:
                assert len(result) == 0, 'Rows with different number of columns detected!'
                continue
            result.append(np.array([float(n) for n in numbers]))
        assert len(result) > 0, 'Unknown file format. Can\'t guess'
        result = np.array(result)
...
def rFactorSp(theory, exp, weight=None, p=2, sub1=False, interval=None, fit=None, normalize=None, divBy='exp', returnSpectra=False):
    ...
    if normalize is not None:
        ...
        if sub1:
            ...
            ye = ye-1
            sub1 = False
        norm = lambda y: integral(e, np.abs(y * np.conj(y)) ** (p2 / 2) * weight)
        yt = yt / norm(yt)
        ye = ye / norm(ye)
...
def fixXraydb():
    if not os.path.exists('/home/sg'):
        return
    xraydb_spec = importlib.util.find_spec('xraydb')
    xraydb_py_path = f'{os.path.dirname(xraydb_spec.origin)}/xraydb.py'
    # assert len(xraydb_file_path) == 1
    sql_file = os.path.dirname(xraydb_py_path) + '/xraydb.sqlite'
    new_sql_file = '/tmp/xraydb.sqlite'
    assert os.path.exists(xraydb_py_path)
    with open(xraydb_py_path) as f:
        s = f.read()
    fix = f"dbname='{new_sql_file}'"
    if os.path.exists(new_sql_file) and fix in s:
        return
    if not os.path.exists(new_sql_file):
        shutil.copyfile(sql_file, new_sql_file)
    if fix not in s:
        s = s.replace("dbname='xraydb.sqlite'", fix)
        with open(xraydb_py_path, 'w') as f:
            f.write(s)
