...
class Estimator:
    ...
    def predictRDF(self, spectrum, folderToSaveResult, atoms=None, smooth=True, extraMolecules={}):
        ...
        xanesAbsorb = self.prepareSpectrumForPrediction(spectrum, smooth)
        spectrum = utils.Spectrum(self.xanes_energy, xanesAbsorb)
        ...
        if extraMolecules is not None:
            cvRes = KFoldCrossValidation(regressor, sample.spectra, dists, 4)
...
def predictRDF(spectrum, spectra, dists, regressor=None, fileName='rdf.png', plotMoreFunction=None):
    ...
    if isinstance(spectra, pd.DataFrame):
        ...
        spectrum = spectrum.changeEnergy(energy, inplace=False)
    else:
    ...
    plotting.plotToFile(arg, predictedRdf, {'lw':2, 'color':'r', 'label':'predicted RDF'}, predictedDists, np.zeros(predictedDists.size), {'fmt':'rP', 'ms':15, 'label':'predicted dists'}, fileName=fileName, plotMoreFunction=plotMoreFunction)
