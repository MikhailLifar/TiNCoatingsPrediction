...
def plotToFile_createfig(*p, axisMatrix=None, axisMatrixKw=None, showInNotebook=False, **kw):
...
def plotToFile_plotOne(ax, p, toSave):
    ...
    for i in range(n):
        if isinstance(p[i*3+2], str):
            ...
            if not utils.isFlask(): ax.plot(p[i*3], p[i*3+1], label=label)
        else:
...
def plotToFile_plotAll(ax, axisMatrix, axisMatrixKw):
    ...
    for i in range(len(axisMatrix)):
        for j in range(len(axisMatrix[0])):
            ...
            setFigureSettings(ax[i][j], **axisMatrixKw[i][j])
    return toSave


def plotToFile_save(fig, toSave, fileName, save_csv, showInNotebook, **kw):
...
def plotToFile(*p, axisMatrix=None, axisMatrixKw=None, fileName=None, save_csv=True, showInNotebook=False, plotWrapper=None, **kw):
    """
    ...
    if plotWrapper is not None:
        ...
        fig, ax, axisMatrixKw, axisMatrix = plotToFile_createfig(*p, axisMatrix=axisMatrix, axisMatrixKw=axisMatrixKw, showInNotebook=showInNotebook, **kw)
        toSave = plotToFile_plotAll(ax, axisMatrix, axisMatrixKw)
        plotToFile_save(fig, toSave, fileName, save_csv, showInNotebook, **kw)
...
def readPlottingFile(fileName):
    ...
    for line in s.split('\n'):
        ...
        while name in d:
            name = f'{name}_{i}'
            i+=1
        if name0 == 'title': d[name] = line.split(':')[1]
...
def plotSample(energy, spectra, colorParam=None, sortByColors=False, fileName=None, alpha=None, colorBarFormat='%.2g', colorBarLabel=None, cmap=None, plotNaN=True, **kw):
    ...
    if colorParam is not None and len(colorParam)>0:
        ...
        if isinstance(colorParam[0], str):
...
def plotMatrix(mat, cmap=None, ticklabelsX=None, ticklabelsY=None, annot=False, fmt='.2g', fileName=None, **kw):
    fig, ax = createfig()
    df = None
    if ticklabelsX is not None:
        assert mat.shape[0] == len(ticklabelsY) and mat.shape[1] == len(ticklabelsX)
...
def plotConfusionMatrix(trueLabels, predictedLabels, labelName, labelMap=None, cmap=None, fileName='', **kw):
    ...
    plotMatrix(conf_mat, cmap=cmap, ticklabelsX=np.unique(trueLabels), ticklabelsY=np.unique(trueLabels), annot=True, fmt='.2g', fileName=fileN, **kw)
